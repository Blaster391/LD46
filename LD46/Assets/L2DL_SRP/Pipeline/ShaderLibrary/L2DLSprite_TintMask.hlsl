#ifndef MYRP_L2DLSPRITETINTMASK_INCLUDED
#define MYRP_L2DLSPRITETINTMASK_INCLUDED
...
VertexOutput L2DLSpriteTintMaskPassVertex (VertexInput input) 
{
	VertexOutput output;
...
FragOutput L2DLSpriteTintMaskPassFragment (VertexOutput input) 
{
    ////  Sample Inputs ////
    ...
    float tintMask = SAMPLE_TEXTURE2D(_TintMask, sampler_TintMask, lightingMaskTexcoord).r;
    ...
    // Target 1 : Color
	output.color = spriteColor * (input.color * tintMask + float4(1, 1, 1, 1) * (1 - tintMask));
...
#endif // MYRP_L2DLSPRITETINTMASK_INCLUDED